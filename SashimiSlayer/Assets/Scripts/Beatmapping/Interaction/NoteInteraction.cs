...
namespace Beatmapping.Interactions
{
    ...
    public class NoteInteraction
    {
        ...
        public NoteInteractionType Type { get; }
        ...
        public BlockPoses BlockPose { get; }

        public bool HideIndicator { get; private set; }

        private readonly TimingWindow _timingWindow;

        public NoteInteraction(NoteInteractionType typeType,
            InteractionFlags interactionFlags,
            BlockPoses blockPose,
        ...
        /// <param name="attemptedNoteInteraction">the interaction attempted by the protag</param>
        /// <param name="blockPose">the block pose protag is using, if relevant</param>
        /// <returns></returns>
        public AttemptResult TryInteraction(
            double attemptTime,
            NoteInteractionType attemptedNoteInteraction,
            BlockPoses blockPose = default)
        {
            ...
            if (attemptedNoteInteraction != Type)
            {
                return new AttemptResult
                {
                    ValidInteraction = false
            ...
            if (Type == NoteInteractionType.Block &&
                blockPose != BlockPose)
            {
